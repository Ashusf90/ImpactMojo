...
// CORRECTED IMPORT: Using absolute path from 'src'
import { usePage, useAuth } from 'context/AppContext';
        ...
        <div className="flex justify-between h-20">
          <div className="flex">
            ...
            <div className="hidden sm:ml-8 sm:flex sm:space-x-8">
              ...
              {user && (
                <>
                  <button onClick={() => setCurrentPage('dashboard')} className={`${currentPage === 'dashboard' ? 'border-blue-600 text-gray-900 dark:text-white' : 'border-transparent text-gray-500'} inline-flex items-center px-1 pt-1 border-b-2 text-sm font-medium`}>Dashboard</button>
                  <button onClick={() => setCurrentPage('about')} className={`${currentPage === 'about' ? 'border-blue-600 text-gray-900 dark:text-white' : 'border-transparent text-gray-500'} inline-flex items-center px-1 pt-1 border-b-2 text-sm font-medium`}>About</button>
                  <button onClick={() => setCurrentPage('faq')} className={`${currentPage === 'faq' ? 'border-blue-600 text-gray-900 dark:text-white' : 'border-transparent text-gray-500'} inline-flex items-center px-1 pt-1 border-b-2 text-sm font-medium`}>FAQ</button>
                </>
              )}
            </div>
          </div>
          <div className="hidden sm:ml-6 sm:flex sm:items-center space-x-4">
            <button onClick={toggleDarkMode} className="p-2 rounded-md text-gray-400"><span className="text-gray-400">{darkMode ? <Sun /> : <Moon />}</span></button>
            {user ? (
